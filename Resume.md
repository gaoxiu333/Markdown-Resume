...
- **分析小程序**
  ...
  *技术栈：*`原生小程序`|`echarts`
  ...
  描述：学生成绩分析系统的小程序版，为了方便教师在移动端上能看到分析系统，提供了分析系统绝大部分功能。

  - 负责开发班级、校级、联级等核心模块的开发，统一鉴权等模块的开发；
  - 统一鉴权、通用方法等封装成 NPM 包管理，方便小程序等多端代码的逻辑的复用。

- **区域数据大屏**

  技术栈：`React`|`React-Router`｜`Webpack`|`ECharts`｜`WebSocket`
